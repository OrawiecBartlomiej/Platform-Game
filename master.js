...
class Coin {
    ...
    static create(pos) {
        ...
        return new Coin(basePos, basePos,
            Math.random() * Math.PI * 2); //The phase of Math.sin’s wave, the width of a wave it produces, is 2π.
    }
}
...
//function provides a succinct way to create an element and give it some attributes and child nodes:
...
DOMDisplay.prototype.scrollPlayerIntoView = function(state){
    ...
    let margin = width/3; //if player is in center 1/3 of screen, the screen doesn't move
...
//works like scrollPanelIntoView
...
CanvasDisplay.prototype.drawBackground = function(level){
    ...
    for(let y = yStart; y < yEnd; y++){
        for (let x = xStart; x < xEnd; x++){
            ...
            this.cx.drawImage(otherSprites, tileX, 0, //only lava or wall
                scale, scale, screenX, screenY,scale, scale);
        }
    }
};
...
CanvasDisplay.prototype.drawPlayer = function(player, x, y, width, height){
    ...
    if(player.speed.x != 0){
     ...
     let tile = 8;  //stay still
    if(player.speed.y != 0){
        tile = 9; //jumping
    }
    else if(player.speed.x != 0){
        tile = Math.floor(Date.now()/60)%8; //update every 60ms
    }
    ...
    this.cx.drawImage(playerSprites, tileX, 0,
...
//checks if rectangle touches given type of gthe grid
Level.prototype.touches = function(pos, size, type){ //changed var to let
...
State.prototype.update = function(time, keys){
    ...
    if(newState.status !== "playing") return newState; //changed
    ...
    for(let actor of actors){               //collisions
        if(actor !== player && overlap(actor, player)){ //changed
...
//let simpleLevel = new Level(GAME_LEVELS);
